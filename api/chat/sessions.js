...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const getSessionMatch = url.match(/^\/api\/chat\/sessions\/([^/]+)$/);
    if (getSessionMatch && method === 'GET') {
      const session = sessions.get(getSessionMatch[1]);
      if (!session) return res.status(404).json({ error: 'Session not found' });
      return res.json({ id: getSessionMatch[1], ...session });
    }

    const postMessageMatch = url.match(/^\/api\/chat\/sessions\/([^/]+)\/messages$/);
    if (postMessageMatch && method === 'POST') {
      const sessionId = postMessageMatch[1];
      let session = sessions.get(sessionId);

      if (!session) {
        session = { title: '新对话', messages: [] };
      ...
      const response = await chat(recentMessages, 'chat', turnCount);
      ...
      return res.json({ response, messages: session.messages });
    }

    const deleteSessionMatch = url.match(/^\/api\/chat\/sessions\/([^/]+)$/);
